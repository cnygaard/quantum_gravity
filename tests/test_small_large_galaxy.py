...
def test_quantum_correction_scaling():
    ...
    small_galaxy = StellarDynamics(
        orbital_velocity=150,           # km/s
        radius=1e3 * SI_UNITS['ly_si'], # meters
        mass=1e8 * SI_UNITS['M_sun_si'] # kg
    )
    radius_planck = small_galaxy.radius * CONVERSIONS['length_to_planck']
    mass_planck = small_galaxy.mass * CONVERSIONS['mass_to_planck']
    beta = PLANCK_UNITS['l_p'] / radius_planck
...
def test_geometric_origin():
    ...
    def calc_with_dim(dim):
        # Fix scaling to be proportional to sqrt(dim)
        return galaxy.compute_geometric_enhancement() * np.sqrt(dim/8)
    
    enh_24d = calc_with_dim(24)
    enh_8d = calc_with_dim(8)
